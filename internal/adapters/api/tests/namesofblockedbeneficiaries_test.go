...
func TestGetBlockedUsers(t *testing.T) {
	...
	t.Run("testing bad request", func(t *testing.T) {
		...
		mockDb.EXPECT().GetBlockedBeneficiary().Return(nil, errors.New("internal server error"))
		rw := httptest.NewRecorder()
		req, _ := http.NewRequest(http.MethodGet, "/api/v1/admin/blockedusers", strings.NewReader(string(bytes)))
	...
	t.Run("testing successful request", func(t *testing.T) {
		mockDb.EXPECT().TokenInBlacklist(gomock.Any()).Return(false)
		mockDb.EXPECT().FindAdminByEmail(admin.Email).Return(&admin, nil)
		mockDb.EXPECT().GetBlockedBeneficiary().Return(beneficiaries, nil)
		rw := httptest.NewRecorder()
		req, _ := http.NewRequest(http.MethodGet, "/api/v1/admin/blockedusers", strings.NewReader(string(bytes)))
		req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", *accToken))
		router.ServeHTTP(rw, req)
		assert.Equal(t, http.StatusOK, rw.Code)
		assert.Contains(t, rw.Body.String(), "blocked users successfully gotten")
	})
}
